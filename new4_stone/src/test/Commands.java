...
public class Commands {
	...
	private class SharedState {
		...
		public void TimeS(String filename)
		{
		...
		public void anomalyleran(String filename)
		{
	...
	public class UploadFileCsv extends Command {
		...
		public void execute() {
			...
			try {
				PrintWriter out = new PrintWriter(new FileWriter("anomalyTest.csv"));
				String line = dio.readText();
				while (!line.equals("done")) {
					out.println(line);
					line = dio.readText();
	...
	public class algoSetting extends Command {
		...
		public void execute() {
			...
			float th = Float.parseFloat(dio.readText());//thshould
			while (th < 0 || th > 1) {
				dio.write("Please choose a value between o and 1\n");
				th = Float.parseFloat(dio.readText());
			}
			sharedState.sd.setMythershold(th);
	...
	public class DetectAnomalies extends Command {
		...
		@Override
		public void execute() {
			sharedState.anomalyleran("anomalyTrain.csv");
	...
	public class AnalyzeResult extends Command {
		...
		public void execute() {
			...
			dio.write("Upload complete\n");
				...
				sharedState.TimeS("anomalyTest.csv");
              ...
              Positive=contiue.size();//size of list is positive

               boolean [] reports=new boolean[contiueError.size()];
